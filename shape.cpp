...
static std::map<QString, int> shape_type_map {
  {"polygon", shape_polygon},
    ...
    {"line", shape_line},
    {"circle", shape_circle},
    {"linestrip", shape_linestrip}
};

std::vector<QString> multi_points_shape {
    "polygon",
    "linestrip"
};
...
Shape::Shape(const QString& label, const QColor& line_color,
  ...
  _highlightSettings.insert(std::make_pair(NEAR_VERTEX, std::make_pair(4.0, P_ROUND)));
  _highlightSettings.insert(std::make_pair(MOVE_VERTEX, std::make_pair(1.5, P_SQUARE)));
...
void Shape::shape_type(const QString value) {
  if (std::find(valid_shape.begin(), valid_shape.end(), value) != valid_shape.end())
  {
...
bool Shape::canAddPoint() {
  return find(multi_points_shape.begin(), multi_points_shape.end(), shape_type_) != multi_points_shape.end();
}
...
QRectF Shape::getRectFromLine(const QPointF& pt1, const QPointF& pt2) {
  auto x1 = pt1.x(); auto y1 = pt1.y();
  auto x2 = pt2.x(); auto y2 = pt2.y();
